...
namespace _03_SQLInjectionDataAccessLayer
{
    public class SalesDb : IDisposable
    {
        ...
        //new
        public List<Sale> ListShowAllSales()
        {
            string cmdText = @"select Id as [Sale Id],BuyerId,SellerId,Price,SaleDate
                                from Sales";
        ...
        public List<Buyer> ListShowAllBuyers()
        {
            string cmdText = @"select Id,Name,Surname from Buyers";
            SqlCommand command = new SqlCommand(cmdText, sqlConnection);
            SqlDataReader reader = command.ExecuteReader();
        ...
        public List<Seller> ListShowAllSellers()
        {
            string cmdText = @"select Id,Name,Surname from Sellers";
            SqlCommand command = new SqlCommand(cmdText, sqlConnection);
            SqlDataReader reader = command.ExecuteReader();
        ...
        public List<FullSale> ListShowFullSales()
        {
            string cmdText = @"select s.Id as [Transaction Id],sl.Id,sl.Name,sl.Surname,s.Price,s.SaleDate,b.Id,b.Name,b.Surname
                                from Sales as s join Buyers as b on s.BuyerId=b.Id
                                				join Sellers as sl on s.SellerId=sl.Id";
            ...
            List<FullSale> sales = new List<FullSale>();

            while (reader.Read())
            {
                sales.Add(new FullSale()
                {
                    Id = reader.GetInt32(0),
                    SellerID = reader.GetInt32(1),
                    SlName = reader.GetString(2),
                    SlSurname = reader.GetString(3),
                    Price = reader.GetInt32(4),
                    SaleDate = reader.GetDateTime(5),
                    BuyerID = reader.GetInt32(6),
                    ByName = reader.GetString(7),
                    BySurname = reader.GetString(8)
                });
            }
            reader.Close();
            return sales;
